...
from src.crypto.execution_algo.tools.constants import Accounts, Exchanges
...
if __name__ == "__main__":
    ...
    if TRADING_PARAMS["exchange"].upper() not in Exchanges.__members__:
        print("Exchange not available")
        print(f"Select from the following: {list(Exchanges)}")
    elif TRADING_PARAMS["account_name"].upper() not in Accounts.__members__:
        print("Account Selected not available")
        print(f"Select from the following: {list(Accounts)}")
