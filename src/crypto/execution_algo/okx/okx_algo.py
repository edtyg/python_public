...
from src.crypto.execution_algo.helper.helper_okx import HelperOkx
from src.crypto.execution_algo.tools.constants import AlgoTypes, Constants


class OkxAlgoTrading(HelperOkx):
    """
    Args:
    ...
    def okx_trading_algo(self, order_params: dict) -> None | str:
        """
        ...
        base_ccy_balance = HelperOkx.get_spot_balance(
            self, trading_params[Constants.BASE_CURRENCY]
        )
        quote_ccy_balance = HelperOkx.get_spot_balance(
            self, trading_params[Constants.QUOTE_CURRENCY]
        )
        base_ccy_price = HelperOkx.get_spot_coin_price(
            self, trading_params[Constants.BASE_CURRENCY]
        )
        quote_ccy_price = HelperOkx.get_spot_coin_price(
            self, trading_params[Constants.QUOTE_CURRENCY]
        )
