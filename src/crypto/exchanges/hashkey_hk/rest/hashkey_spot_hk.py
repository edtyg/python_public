...
from src.crypto.exchanges.hashkey_hk.rest.hashkey_client_hk import HashkeyExchange
...
class HashkeySpot(HashkeyExchange):
    ...
    def get_fund_statement(self, params: Optional[Dict] = None) -> dict:
        """Signed API call
        https://hashkeypro-apidoc.readme.io/reference/get-fund-statement

        Args:
            params (Optional[Dict], optional): Defaults to None
            PARAMETER       TYPE    REQUIRED    DESCRIPTION
            accountId	    LONG		        Account ID
            type            STRING  YES         "trade",
                                                "deposit",
                                                "fiatDeposit",
                                                "withdraw",
                                                "fiatWithdraw",
        ...
        Returns:
            dict
        """
        endpoint = "/api/v1/account/balanceFlow"
        response = self._get_private(endpoint, params)
        return response
    ...
    ### OPT ###
    def get_opt_trades(self, params: Dict):
        """"""
        endpoint = "/api/exchange/cfs/opt/trade/records"
        response = self._get_private(endpoint, params)
        return response
